    ...
    {#if $stories[selectedS].shape === "circle"}
      <ul>
        {#each $stories[selectedS].events_of_interest as eoi, index}
          <li in:fade={{delay: index*50 + 100}} out:fade={{delay: index*50}} on:click={() => {$selectedEoI = index}} class:selected={$selectedEoI === index}>
            <div class="number">
              {#if $selectedEoI === index}
                <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg" transition:fade>
      ...
      <ul>
        {#each $stories[selectedS].events_of_interest as eoi, index}
          <li in:fade={{delay: index*50 + 100}} out:fade={{delay: index*50}} on:click={() => {$selectedEoI = index}} class:selected={$selectedEoI === index}>
            <div class="number">
              {#if $selectedEoI === index}
                <svg width="31" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg" transition:fade>
      ...
      <ul>
        {#each $stories[selectedS].events_of_interest as eoi, index}
          <li in:fade={{delay: index*50 + 100}} out:fade={{delay: index*50}} on:click={() => {$selectedEoI = index}} class:selected={$selectedEoI === index}>
            <div class="number">
              {#if $selectedEoI === index}
                <svg width="34" height="30" viewBox="0 0 34 30" fill="none" xmlns="http://www.w3.org/2000/svg" style="transform: translate(-2px, -2px)" transition:fade>
