<script lang="ts">
  ...
  function calculateOffset(year: number): number {
    ...
    return year < $yearLimits.min
      ? ((year - 1500) / beforeTimeDifference / 100) * beforeTimeOffset * 100
      : beforeTimeOffset + ((year - $yearLimits.min) / timeDifference) * (100 - beforeTimeOffset);
  }
...
{#if ready}
  <div
    class="timeline"
    in:fade={{ duration: 1000, delay: $firstLoad ? 4500 : 0, ease: sineOut }}
    out:fade={{ duration: 500, ease: sineIn }}
  >
    <!--Story Usage-->
      ...
      {#if $selectedStory === index}
        {#each story.usage as usage}
          <div
            ...
            class:round-left={usage.start === 1500}
            style="left: {calculateOffset(usage.start)}%; width: calc({calculateOffset(usage.end) - calculateOffset(usage.start)}% - 2px)"
            transition:fade={{ duration: 300, ease: sineInOut }}
          />
        {/each}
      {/if}
    {/each}
    ...
    <div class="displayYear removeLine number">1500</div>
    ...
    {#each $yearChanges as y}
      {#if y !== $yearLimits.max}
        <div
          ...
          data-year={y}
          use:tooltip
          style="left: {calculateOffset(y)}%; transform: {y === $yearLimits.min ? 'translate(calc(-50%), 0)' : 'translate(-50%, 0)'}"
          on:click={(e) => {
    ...
    <div class="reference_description">Vorhandene Referenzkarten</div>
  </div>
{/if}
{#if dragging}
  <div class="grabboard" />
