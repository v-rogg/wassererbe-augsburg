  ...
  {#each $stories as story, sIndex}
    {#each story.events_of_interest as EoI, eoiIndex}
      {#if sIndex === $selectedStory && eoiIndex === $selectedEoI}
        ...
        >
          <div class="zoneIcon">
            <img src="/legend/{$stories[$selectedStory].events_of_interest[$selectedEoI].zone_type}.svg" alt="" />
          </div>
        ...
        {#if $selectedEoI + 1 < $stories[$selectedStory].events_of_interest.length}
          <div class="nextUp">
            <button
              ...
              on:click={() => {
                eoiIndex + 1 > $selectedEoI ? ($storyDirection = StoryDirection.Down) : ($storyDirection = StoryDirection.Up);
